...
```shell
kubectl port-forward svc/ahoy-argocd-server -n ahoy 8090:443
```

The API server can then be accessed using the localhost:8090

The initial password is autogenerated to be the pod name of the Argo CD API server. This can be retrieved, for example, with the command:
```shell
kubectl get pods -n ahoy -l app.kubernetes.io/name=argocd-server -o name | cut -d'/' -f 2
```
